...
for i in range(num_layers):
    histories.append({'longest_similar':[], 'shortest_distinct': []})
...
if __name__ == "__main__":
    ...
    for	layer_index in range(0, num_layers-1):
        ...
        for epoch_index in range(num_epoches):
            ...
            print("layer:", layer_index+1, " epoch:", epoch_index,
                " longest similar:", selected_similar_index[1],
                " shortest distinct:", selected_distinct_index[1])
            histories[layer_index]['longest_similar'].append(selected_similar_index[1])
            histories[layer_index]['shortest_distinct'].append(selected_distinct_index[1])


            ## random select longest/shortest distance
            i = random.randint(0, len(similar_pairs) - 1)
            p = similar_pairs[i]
            distance = cal_dis(p, output_dimension, cur_layer)
            selected_similar_index[0] = i
            selected_similar_index[1] = distance

            i = random.randint(0, len(distinct_pairs) - 1)
            p = distinct_pairs[i]
            distance = cal_dis(p, output_dimension, cur_layer)
            selected_distinct_index[0] = i
            selected_distinct_index[1] = distance
            ...
            # Distinct class pair
            # print("selected:", selected_similar_index[0], len(selected_similar_index))
            # print("len", len(distinct_pairs))
            selected_pair = distinct_pairs[selected_distinct_index[0]]
            for i in range(output_dimension):
                left_curr_val = cur_layer[selected_pair[0] * max_dimension + i]
                right_curr_val = cur_layer[selected_pair[1] * max_dimension + i]
                weight_scales_left[i] = (left_curr_val - right_curr_val) * (1.0 - left_curr_val ** 2)
...
pickle.dump(histories, open('q2_history_random.pickle', 'wb'))